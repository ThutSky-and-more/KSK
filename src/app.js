const K = (() => {
  ...
  async function home() {
    ...
    else if (el) el.innerHTML = `<span class="tag">Freitigs-Flashback</span><h3>No kei Flashback.</h3><p>Sobald de erscht Beitrag freigegeh isch, chunnt jede Friitig automatisch öppis zrugg.</p>`;
  }
  ...
  async function listing(category = null) {
    ...
    const render = () => {
      ...
      grid.innerHTML = filtered.length ? filtered.map(postCard).join('') : `<div class="glass empty-state empty-wide"><strong>No nüt da.</strong><span>${q ? 'Mit dere Suechi hämmer nüt gfunde.' : 'Da isch no kei freigegebene Iitrag.'}</span>${meta ? `<a class="btn btn-primary" href="${meta.create}">Selber ${esc(meta.label)} iiträge →</a>` : '<a class="btn btn-primary" href="/schreiben/">Öppis iiträge →</a>'}</div>`;
  ...
  async function flashback() {
    ...
    if (!chosen) { mount.innerHTML='<div class="empty-state"><strong>No kei Flashback.</strong><span>Zerscht müend freigegebni Beiträg da sii.</span><a class="btn btn-primary" href="/schreiben/">Öppis iiträge →</a></div>'; return; }
